...
type MultipleDataDropDownMenuProps = {
    ...
    onSelectionChange?: (selectedBloomActiviesIndex: number, selectedActivityIndex: number, event?: any) => void;
...
export default function MultipleDataDropDownMenu({
    ...
    useEffect(() => {
        if (itemIndex.length === 0) {
            ...
            const longTitle = itemIndex
                .map(
                    (indexArray: number[], i: number) =>
                        indexArray.map(
                            (index: number) =>
                                multipleData[i]?.activities[index]?.title ||
                                defaultMenuTitle
                        )
                )
                .join(', ');
    ...
    //   }
    // };
    const handleMenuItemClick = (data: MultipleArrayProps, indexData: number, selectedItemIndex: number) => {
    ...
    const handleHighlight = () => {
        ...
        const isHighlighted = itemIndex.map((items: number[]) => items.length > 0 ? false : true)
                    ...
                    {hydrated &&
                        multipleData?.map((data: MultipleArrayProps, indexData: number) => (
                            <MenuOptionGroup key={indexData} w='fit-content'>
                                {hydrated &&
                                    data?.activities?.map((activity: ArrayProps, index: number) => (
                                                    ...
                                                    {isCheckBoxNeeded && (
                                                        <Flex flex="1" justify={'flex-end'}>
                                                            <Checkbox
                                                                ...
                                                                colorScheme="yellow"
                                                                onChange={() => handleMenuItemClick(data, indexData, index)}
                                                                isChecked={
                                                                    itemIndex[indexData]?.includes(index) || false
                                                                }
                                                            />
                                                        </Flex>
                                                    )}
