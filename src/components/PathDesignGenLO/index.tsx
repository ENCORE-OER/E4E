...
import { EducationContextEnum, YourExperienceEnum as EducatorExperienceEnum, GroupDimensionEnum, LearnerExperienceEnum } from '../../types/encoreElements/PathDesignElement/enums';
...
export default function PathDesignGenLO(
    ...
    const mapOptionToNumber = (option: Option | null, enumObject: any): number => {
    ...
    const handleGenerateLO = async (e: any) => {
        ...
        if (
            ...
            addToast({
                message: 'Please fill out all the required fields before generating learning objectives.',
                type: 'warning',
            });
            ...
            try {
                ...
                if (numberOfLO > 0) {
                    ...
                    console.log('Skills: ', selectedSkillConceptsTags?.map((skill: SkillItemProps) => skill.label).join(', '));
                    ...
                    for (let i = 0; i < numberOfLO; i++) {
                        ...
                        const resp = await postGenerateLearningObjective(
                            ...
                            mapOptionToNumber(selectedEducatorExperience, EducatorExperienceEnum),  // educatorExperience
                            mapOptionToNumber(selectedLearnerExperience, LearnerExperienceEnum), // learnerExperience
                            ...
                            selectedSkillConceptsTags.map((skill: SkillItemProps) => skill.label).join(', '), // skills
                    ...
                    addToast({
                        message: 'Please enter the number of learning objectives to generate.',
                        type: 'warning',
                    });
                }
            } catch (error) {
    ...
    const cutResponse = (resp: string) => {
        ...
        const startIndex = resp.indexOf(`Temperature: ${TEMPERATURE_GEN_LO_API}`) + `Temperature: ${TEMPERATURE_GEN_LO_API}`.length + 1;
    ...
    return (
        <>
                ...
                >
                    ...
                    >
                        <Textarea
                            ...
                            typeof='number'
                            errorBorderColor={numberOfLO === 0 ? '1.5px solid #bf5521ff' : 'none'}
                            value={numberOfLO}
            ...
            <Flex p='15px' direction='column'
                border={isHighligted && generatedLOs.length > 0 && selectedLO.length === 0
